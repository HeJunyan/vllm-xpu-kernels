 ...
 public:
  ...
  struct Arguments {
    ...
    RasterOrderOptions raster_order =
        RasterOrderOptions::AlongM;  // AloneN by input
  ...
  CUTLASS_DEVICE explicit PersistentTileSchedulerXe4Group(
    ...
    /* if
     * (is_tuple<decltype(cute::shape<0>(params_.group_params.problem_shapes_.get_problem_shape(0)))>::value
     * || */
    /*     is_tuple<decltype(cute::shape<1>(params_.group_params.problem_shapes_.get_problem_shape(0)))>::value)
     * { */
    /*   ctas_along_m =
     * cute::size(cute::ceil_div(cute::shape<0>(params_.group_params.problem_shapes_.get_problem_shape(0)),
     * scheduler_params.group_params.cta_shape_.m())); */
    /*   ctas_along_n =
     * cute::size(cute::ceil_div(cute::shape<1>(params_.group_params.problem_shapes_.get_problem_shape(0)),
     * scheduler_params.group_params.cta_shape_.n())); */
    /* } */
    /* else { */
    /*   ctas_along_m =
     * scheduler_params.group_params.divmod_cta_shape_m_.divide(cute::shape<0>(params_.group_params.problem_shapes_.get_problem_shape(0))
     * +  scheduler_params.group_params.divmod_cta_shape_m_.divisor - 1); */
    /*   ctas_along_n =
     * scheduler_params.group_params.divmod_cta_shape_n_.divide(cute::shape<1>(params_.group_params.problem_shapes_.get_problem_shape(0))
     * +  scheduler_params.group_params.divmod_cta_shape_n_.divisor - 1); */
    /* } */
    /* auto problem_blocks_m = round_up(ctas_along_m, (1 <<
     * params_.group_params.log_swizzle_size_) *
     * params_.group_params.cluster_shape_.m()); */
    /* auto problem_blocks_n = round_up(ctas_along_n, (1 <<
     * params_.group_params.log_swizzle_size_) *
     * params_.group_params.cluster_shape_.n()); */
  ...
  void advance_to_next_work(
      int32_t* shared_counter = nullptr, uint32_t advance_count = 1) {
#ifndef Dynamic
    ...
    if (item.get_local_linear_id() == 0) {
      shared_counter[0] = cutlass::atomicAdd(
          scheduler_params.global_task_counter_, 1UL * advance_count);
  ...
  WorkTileInfo fetch_next_work(
      ...
      int32_t* shared_counter = nullptr,
    ...
    if (shared_counter[1] > 0) {
      current_work_linear_idx_ =
          total_grid_size_ + shared_counter[0] - shared_counter[1];
      item.barrier(sycl::access::fence_space::local_space);
      if (item.get_local_linear_id() == 0) {
        shared_counter[1]--;
      }
