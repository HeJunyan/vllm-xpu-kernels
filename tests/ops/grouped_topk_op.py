...
from tests.ops.topk_op import numpy_topk
...
def grouped_topk(
    ...
    if e_score_correction_bias is not None:
        ...
        group_scores = (numpy_topk(
            scores.view(num_token, num_expert_group, -1), 2,
            dim=-1)[0].sum(dim=-1))
    else:
        group_scores = scores.view(num_token, num_expert_group,
                                   -1).max(dim=-1).values  # [n, n_group]
    _, group_idx = numpy_topk(group_scores, topk_group,
                               dim=-1)  # [n, top_k_group]
    ...
    tmp_scores = scores.masked_fill(~score_mask.bool(),
                                    float("-inf"))  # [n, e]
    if e_score_correction_bias is not None:
        _, topk_ids = numpy_topk(tmp_scores, topk, dim=-1)
        # Use original unbiased scores for the routing weights
        topk_weights = original_scores.gather(1, topk_ids)
    else:
        topk_weights, topk_ids = numpy_topk(tmp_scores, topk, dim=-1)
    if renormalize:
