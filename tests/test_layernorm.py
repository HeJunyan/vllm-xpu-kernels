...
def test_rms_norm(
    ...
    weight = layer_k.weight.data if has_weight else None
    if residual is not None:
        opcheck(torch.ops._C.fused_add_rms_norm,
                (_to_kernel(x), _to_kernel(residual), weight,
                 layer_k.variance_epsilon))
    else:
        opcheck(torch.ops._C.rms_norm,
                (out.to(KERNEL_DEVICE), _to_kernel(x), weight,
                 layer_k.variance_epsilon))
